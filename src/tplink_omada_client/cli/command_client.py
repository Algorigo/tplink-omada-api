        ...
        if (
            ...
            if args["dynamic_ip"]:
                ...
                if not args["network"]:
                    raise ArgumentError(
                        args["network"],
                        "Network ID must be specified when reserving an IP address",
                    )
                settings.fixed_address = OmadaClientFixedAddress(
                    network_id=args["network"], ip_address=args["fixed_ip"]
                )
...
def arg_parser(subparsers: _SubParsersAction) -> None:
    ...
    client_parser = subparsers.add_parser(
        "client", help="Shows details of a client known to Omada controller"
    )
    ...
    lock_grp.add_argument(
        "-l",
        "--lock-to-ap",
        help="Lock the client to the specified access point(s)",
        metavar="MACs",
        type=list_of_strings,
    )
    lock_grp.add_argument(
        "-u", "--unlock", help="Unlock the client", action="store_true"
    )
    ...
    fixed_ip_en_dis_grp.add_argument(
        "-ip", "--fixed-ip", help="Reserve the client's IP address"
    )
    fixed_ip_en_dis_grp.add_argument(
        "-dyn",
        "--dynamic-ip",
        help="Remove the client's IP reservation",
        action="store_true",
    )
    fixed_ip_grp.add_argument("-n", "--network", help="Network ID for reservation")

    client_parser.add_argument(
        "-d", "--dump", help="Output raw client information", action="store_true"
    )
